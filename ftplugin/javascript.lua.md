...
This code shows how to use tree-sitter to improve your workflow.

As you type `awa(it)` it will make sure there's an `async` keyword wrapping the
function body.
...
local function add_async()
  ...
  local text_before_cursor = vim.fn.getline('.'):sub(col - 5, col - 1)
...
-- if t is pressed in insert mode
-- look for `awai` as prefix
vim.keymap.set('i', 't', add_async, { buffer = true })

vim.keymap.set('n', '<leader>rr', ':so ~/.config/nvim/ftplugin/javascript.lua<cr>')
```
