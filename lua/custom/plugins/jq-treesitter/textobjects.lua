...
function M.find_value_node(node)
  ...
  if node_type == 'object' or node_type == 'array' or 
     node_type == 'string' or node_type == 'number' or
     node_type == 'true' or node_type == 'false' or node_type == 'null' then
    return node
  end
...
function M.setup_textobjects(bufnr)
  ...
  -- Inner value
  vim.keymap.set({'x', 'o'}, 'ij', function() M.select_inner_value() end,
    vim.tbl_extend('force', opts, { desc = 'inner JSON value' }))
    
  -- Around value
  vim.keymap.set({'x', 'o'}, 'aj', function() M.select_around_value() end,
    vim.tbl_extend('force', opts, { desc = 'around JSON value' }))
    
  -- Inner pair
  vim.keymap.set({'x', 'o'}, 'ik', function() M.select_inner_pair() end,
    vim.tbl_extend('force', opts, { desc = 'inner JSON key-value pair' }))
    
  -- Around pair
  vim.keymap.set({'x', 'o'}, 'ak', function() M.select_around_pair() end,
    vim.tbl_extend('force', opts, { desc = 'around JSON key-value pair' }))
end
