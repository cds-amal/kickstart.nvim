...
function M.get_navigable_positions(bufnr)
  ...
  local nav = require('custom.plugins.jq-treesitter.treesitter-nav')
...
function M.setup_motions(bufnr)
  ...
  -- Navigation motions
  vim.keymap.set('n', ']j', function() M.goto_next_navigable(bufnr) end, 
    vim.tbl_extend('force', opts, { desc = 'Next JSON navigable item' }))
  vim.keymap.set('n', '[j', function() M.goto_prev_navigable(bufnr) end,
    vim.tbl_extend('force', opts, { desc = 'Previous JSON navigable item' }))
end
