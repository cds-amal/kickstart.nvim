...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>sm', builtin.git_status, { desc = '[S]earch Git [M]odified' })
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('FileType', {
        ...
        callback = function()
          ...
          local function split_at_dashes(confirm)
            local pattern = [[\(\s\{1,}\)\(-\+\)]]
          ...
          local function join_with_treesitter()
            ...
            -- print('Starting with node type:', command_node:type())
            ...
            while command_node and command_node:type() ~= 'command' do
              ...
              command_node = parent
              print('Traversed to node type:', command_node:type())
            end
            ...
            local start_row, start_col, end_row, end_col = command_node:range()

            -- Debug logging
            -- print(string.format('Range: start_row=%d, start_col=%d, end_row=%d, end_col=%d', start_row, start_col, end_row, end_col))

            -- Log the number of children nodes
            local child_count = 0
            for _ in command_node:iter_children() do
              child_count = child_count + 1
            end
            -- print('Number of child nodes:', child_count)

            -- Extract all arguments from the command
            local command_parts = {}

            -- Log each child node type and text and collect ALL parts
            for child in command_node:iter_children() do
              local child_type = child:type()
              local child_text = vim.treesitter.get_node_text(child, 0)
              -- print(string.format('Child type: %s, text: %s', child_type, child_text))

              -- Add ALL child nodes to the command parts
              table.insert(command_parts, child_text)
            end

            -- Join all parts with a single space
            local joined_line = table.concat(command_parts, ' ')
            -- print('Joined command:', joined_line)
            ...
            -- Print debug info
            -- print(string.format('Command joined from line %d to %d', start_row + 1, end_row + 1))
          end
          -- Create buffer-local mappings for splitting
          vim.keymap.set('n', '<localleader>s', function()
            split_at_dashes(false)
          end, { buffer = true, silent = true, desc = 'Split at dashes' })

          vim.keymap.set('n', '<localleader>sq', function()
            split_at_dashes(true)
          ...
          vim.api.nvim_buf_create_user_command(0, 'Split', function()
            local pattern = [[\(\s\{1,}\)\(-\+\)]]
            local replacement = [[ \\\r\t\t\2]]
            vim.cmd('s:' .. pattern .. ':' .. replacement .. ':g | noh | retab')
          end, { desc = 'Split command at dashes' })

          vim.api.nvim_buf_create_user_command(0, 'Splitq', function()
            local pattern = [[\(\s\{1,}\)\(-\+\)]]
            local replacement = [[ \\\r\t\t\2]]
            vim.cmd('s:' .. pattern .. ':' .. replacement .. ':cg | noh | retab')
          end, { desc = 'Split command at dashes (confirm)' })

          -- Add the toggle functionality with TreeSitter-aware joining
          -- Add the toggle functionality with improved TreeSitter-aware joining
          vim.keymap.set('n', '<leader>tj', function()
            local line = vim.fn.getline '.'
            if line:match '\\%s*$' or vim.fn.getline(vim.fn.line '.' - 1):match '\\%s*$' then
              -- If current line or previous line has a continuation character, join
              join_with_treesitter()
            else
              ...
              if cmd:match '%s%-' then
                ...
                for i = 1, #cmd do
                  ...
                  if (char == '"' or char == "'") and (i == 1 or cmd:sub(i - 1, i - 1) ~= '\\') then
  ...
  { -- Highlight, edit, and navigate code
    ...
    opts = {
      ensure_installed = { 'bash', 'c', 'cue', 'diff', 'html', 'lua', 'luadoc', 'markdown', 'markdown_inline', 'query', 'vim', 'vimdoc' },
